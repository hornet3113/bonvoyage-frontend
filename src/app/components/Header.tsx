...
// checando si si se hixo bien el efecto vidrio
    ...
    // ── Light variant (app pages: fondo blanco, misma estructura que glass) ──────
    if (isLight) {
        return (
            <div className="w-full flex items-center justify-between gap-4 px-5 md:px-10 py-3 bg-white border-b border-gray-100 text-xs font-medium uppercase">
                ...
                {/* Nav links */}
                <ul className="flex items-center gap-5 text-[11px] font-medium uppercase tracking-wider text-gray-500 flex-1 justify-center">
                    {menus.map(({ label, href }) => {
                        const isAnchor = href.includes("#");
                        const anchorId = isAnchor ? href.split("#")[1] : null;
                        const isActive = pathname === href;
                        ...
                        return (
                            <motion.li
                                layout
                                key={href}
                                className={`inline-block cursor-pointer transition-colors duration-200 hover:text-gray-900 ${isActive ? "text-blue-500 border-b border-blue-400" : ""}`}
                                whileTap={{ scale: 0.93 }}
                            >
                                {isAnchor ? (
                                    <a href={href} onClick={handleAnchorClick}>{label}</a>
                ...
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    >
                        <motion.div
                            layout
                            className="flex items-center rounded-full border border-gray-200 bg-gray-100 overflow-hidden"
                            style={{ paddingLeft: searchOpen ? "0.75rem" : 0 }}
                            transition={{ duration: 0.25, ease: "easeOut" }}
                        >
                            <AnimatePresence>
                                {searchOpen && (
                                    <motion.input
                                        ...
                                        onChange={(e) => setQuery(e.target.value)}
                                        placeholder="Buscar destino..."
                                        className="bg-transparent outline-none text-[11px] text-gray-700 placeholder:text-gray-400 pr-1"
                                    />
                                )}
                            </AnimatePresence>
                            <button
                    ...
                    <SignedOut>
                        <SignInButton mode="modal">
                            <button data-testid="sign-in-button" className="px-3 py-1 rounded border border-gray-400 text-gray-700 hover:bg-gray-100 transition duration-300">
                                Iniciar sesión
                            </button>
                        </SignInButton>
                        <SignUpButton mode="modal">
    ...
    // ── Dark variant (solo landing page) ─────────────────────────────────────────
    const containerClass = "absolute mt-5 flex w-full flex-wrap items-center justify-between gap-2 px-5 text-xs font-medium uppercase opacity-90 md:px-10";
                ...
                {menus.map(({ label, href }) => {
                    ...
                    return (
                        <motion.li
                            layout
                            key={href}
                            className={`${pathname === href ? "border-b-2 border-b-blue-500" : ""} inline-block cursor-pointer border-b-blue-500 transition duration-300 ease-in-out hover:border-b-2 hover:text-white`}
                            whileTap={{ scale: 0.93 }}
                        >
                            {isAnchor ? (
                                <a href={href} onClick={handleAnchorClick}>{label}</a>
